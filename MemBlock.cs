 ...
 */
public class MemBlock : IComparable, ICloneable, ICopyable
{
    ...
    public int CopyTo(byte[] dest, int offset_into_dest)
    {
        if (_length != 0)
        {
            Array.Copy(_buffer, _offset, dest, offset_into_dest, _length);
        }
        return _length;
    }
    ...
    public override int GetHashCode()
    {
        ...
        //Use at most 4 bytes:
        int l = Math.Min(Length, 4) + _offset;
        int val = 0;
    ...
    public string GetString(System.Text.Encoding e, int off, int length)
    {
        ...
        {
            throw new IndexOutOfRangeException("Length greater than MemBlock length");
        }
        if (length != 0)
        {
    ...
    {
        return new MemoryStream(_buffer, _offset, _length, false);
    }
    ...
    {
        return Convert.ToBase64String(_buffer, _offset, _length);
    }
    ...
    {
        return string.Format("{0}, length: {1}, hash: {2}", GetType(),
            Length, GetHashCode());
    }
}